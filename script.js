...
/* ============ COBRANÇA RECORRENTE (clientes marcados como "recorrente") ============
   Toda vez que o painel é aberto, confere se cada cliente recorrente já tem um
   lançamento financeiro para o mês atual. Se não tiver, cria um automaticamente,
   usando o valor do serviço mais recente vinculado a esse cliente. Isso continua
   acontecendo mês a mês até a pessoa desmarcar "Cliente recorrente". */
...
async function gerarLancamentosRecorrentes(){
  ...
  STATE.clientes.filter(c => c.recorrente === 'sim').forEach(cliente => {
    const servicosCliente = STATE.servicos.filter(s => s.clienteId === cliente.id);
    if(!servicosCliente.length) return;
    const servicoRef = servicosCliente[servicosCliente.length - 1]; // serviço mais recente desse cliente
    const valorNum = parseFloat(servicoRef.valor) || 0;
    if(valorNum <= 0) return;

    const jaExiste = STATE.financeiro.some(f => f.clienteRecorrenteId === cliente.id && f.mesReferencia === mesAtual);
    if(jaExiste) return;
    ...
    STATE.financeiro.push({
      ...
      tipo: 'receita',
      descricao: `Mensalidade recorrente – ${cliente.nome}`,
      valor: valorNum,
...
const FORM_FIELDS = {
  clientes: () => [
    ...
    {k:'recorrente', label:'Cliente recorrente', type:'select', options:[['nao','Não'],['sim','Sim — gerar cobrança mensal automaticamente']]},
...
const CLIENTE_FORM_SECTIONS = [
  ...
  { title:'Relacionamento comercial', icon:ICONS.dollar, rows:[['status','responsavelId'],['clienteDesde','receitaMensal'],['recorrente']] },
...
const FORM_LAYOUT = {
  clientes: [['nome','cnpj'],['contatoNome','telefone'],['email','cidade'],['status','responsavelId'],['clienteDesde','receitaMensal'],['recorrente'],['proximaAcaoTitulo'],['proximaAcaoObs'],['observacoes']],
...
async function saveForm(mod, editId){
  ...
  if(mod==='servicos'){
    ...
    const existingFin = STATE.financeiro.find(f=>f.servicoId===obj.id);
    if(valorNum > 0){
