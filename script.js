...
/* ============ COBRANÇA RECORRENTE (serviços marcados como "recorrente") ============
   ...
   lançamento financeiro para o mês atual. Se não tiver, cria um automaticamente,
   usando o valor do próprio serviço. Isso continua acontecendo mês a mês até a
   pessoa desmarcar "Serviço recorrente".
   Importante: o lançamento do mês corrente de um serviço recorrente é o MESMO
   lançamento "Serviço: {título}" que o salvamento do serviço já mantém em dia
   (ver saveForm) — aqui só criamos os lançamentos dos meses em que ninguém
   editou o serviço, evitando duplicar "Serviço" + "Mensalidade" para o mesmo mês. */
...
async function gerarLancamentosRecorrentes(){
  ...
  STATE.servicos.filter(s => s.recorrente === 'sim').forEach(servico => {
    ...
    const jaExiste = STATE.financeiro.some(f => f.servicoId === servico.id && f.mesReferencia === mesAtual);
    if(jaExiste) return;
  ...
  if(mudou) await persist('financeiro');
}
...
async function saveForm(mod, editId){
  ...
  if(mod==='servicos'){
    ...
    const mesAtual = mesReferenciaAtual();
    // Serviço recorrente: mantém 1 lançamento por MÊS (não mexe nos meses anteriores).
    // Serviço não-recorrente: mantém sempre 1 único lançamento (comportamento antigo).
    const existingFin = obj.recorrente === 'sim'
      ? STATE.financeiro.find(f=>f.servicoId===obj.id && f.mesReferencia===mesAtual)
    ...
    if(valorNum > 0){
      ...
      finObj.vencimento = obj.prazoEntrega || obj.dataInicio || todayISO();
      if(obj.recorrente === 'sim') finObj.mesReferencia = mesAtual;
