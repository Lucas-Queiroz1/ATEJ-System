...
/* ============ COBRANÇA RECORRENTE (serviços marcados como "recorrente") ============
   O ciclo de cada serviço recorrente é ancorado no DIA em que ele foi iniciado
   (campo "Data de Início"). Ex.: um serviço iniciado em 15/06 vira o ciclo todo
   dia 15 (15/07, 15/08...), e não no dia 1º do calendário.
   ...
   virou e o sistema:
   1) Cria o lançamento financeiro do novo ciclo, com vencimento no "dia âncora",
...
// cicloAtualDoServico(), ancorada no dia de início de cada serviço.)

// Dia do mês em que o serviço "aniversaria" (baseado na Data de Início). Sem
// data de início definida, cai no comportamento antigo: vira no dia 1º.
function diaAncoraDoServico(servico){
  if(servico.dataInicio){
    const dia = parseInt(servico.dataInicio.split('-')[2], 10);
    if(!isNaN(dia) && dia >= 1 && dia <= 31) return dia;
  }
  return 1;
}

// Identifica em qual "ciclo" (formato YYYY-MM, representando o mês em que o
// ciclo atual começou) a data de hoje está, considerando o dia âncora do serviço.
...
async function gerarLancamentosRecorrentes(){
  ...
  STATE.servicos.filter(s => s.recorrente === 'sim').forEach(servico => {
    ...
    STATE.financeiro.push({
      ...
      valor: valorNum,
      vencimento: vencimentoDoCiclo(servico, cicloAtual),
      status: 'pendente',
      dataPagamento: '',
