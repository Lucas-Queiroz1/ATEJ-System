...
async function gerarLancamentosRecorrentes(){
  ...
  STATE.servicos.filter(s => s.recorrente === 'sim').forEach(servico => {
    ...
    // Chegou um novo ciclo para este serviço recorrente: se ele tinha ficado
    // "Concluído" no ciclo passado, reabre como "Aguardando" para este ciclo.
    ...
    STATE.financeiro.push({
      ...
      clienteNome: clienteNome(servico.clienteId),
      tipo: 'receita',
...
const SEARCH_PLACEHOLDER = { clientes:'Buscar cliente...', servicos:'Buscar serviço...', financeiro:'Buscar lançamento...', equipe:'Buscar por nome, função ou setor...', kanban:'Buscar atividades ou clientes...', demandas:'Buscar demanda...' };
...
const FILTER_OPTIONS = {
  ...
  demandas: [['todos','Todos os status'],['aguardando','Aguardando'],['andamento','Em Andamento'],['concluido','Concluído']],
};
...
function openClienteDrawer(id){
  ...
  const servicosHTML = servicos.length ? servicos.map(s=>`
    <div class="drawer-list-item drawer-list-item-clickable" data-drawer-edit-servico="${s.id}">
      ...
      <div class="drawer-list-info"><strong>${s.titulo}</strong><span class="pill ${s.status}" style="margin-top:3px">${servicoStatusLabel(s.status)}</span>${s.recorrente==='sim' ? ` <span class="pill recorrente" style="margin-top:3px">Recorrente</span>` : ''}</div>
  ...
  const openNovaAtividade = () => { drawerToReopen = x.id; closeClienteDrawer(); openModal('servicos', null, { clienteId: x.id }); };
  ...
  if(novoServicoLink) novoServicoLink.addEventListener('click', openNovaAtividade);
    ...
    <div class="servico-card">
      <div class="servico-card-top">
        ...
        <div style="display:flex;gap:6px">
          ${x.recorrente==='sim' ? `<span class="pill recorrente">Recorrente</span>` : ''}
...
function renderFinanceiro(){
  ...
  const totalReceber = receitasEmAberto.reduce((s,x)=>s+Number(x.valor||0),0);
...
function attachContentEvents(){
  ...
  document.querySelectorAll('[data-pay]').forEach(btn=>{
    btn.addEventListener('click', ()=>{ openReceberModal(btn.dataset.id); });
  });
...
const FORM_FIELDS = {
  ...
  financeiro: () => [
    ...
    {k:'multa', label:'Multa (R$)', type:'number'},
    {k:'juros', label:'Juros (R$)', type:'number'},
    {k:'desconto', label:'Desconto (R$)', type:'number'},
    {k:'valorRecebido', label:'Valor recebido (R$)', type:'number'},
    {k:'formaRecebimento', label:'Forma de recebimento', type:'select', options:[['','—'], ...FORMA_RECEBIMENTO_OPTIONS]},
    {k:'contaFinanceira', label:'Conta financeira', type:'select', options:[['','—'], ...CONTA_FINANCEIRA_OPTIONS]},
  ],
  equipe: () => [
  ...
  demandas: () => [
    ...
    {k:'status', label:'Status', type:'select', options:[['aguardando','Aguardando'],['andamento','Em Andamento'],['concluido','Concluído']]},
...
const FORM_LAYOUT = {
  ...
  servicos: [['clienteId'],['titulo'],['descricao'],['valor','recorrente'],['status'],['dataInicio','prazoEntrega'],['responsavelId','progresso']],
  financeiro: [['clienteId','tipo'],['descricao'],['valor','vencimento'],['status','dataPagamento'],['multa','juros'],['desconto','valorRecebido'],['formaRecebimento','contaFinanceira']],
  ...
  demandas: [['titulo'],['descricao'],['solicitanteId','responsavelId'],['prioridade','status'],['prazo']],
};
...
function openModal(mod, id, preset){
  ...
  if(mod==='equipe'){
    ...
    const avisoRestricaoHTML = (!podeEditarFoto && editing) ? `<div class="field" style="font-size:12.5px;color:var(--slate)">Você só pode atualizar atividades, prazo e carga de trabalho deste colaborador. Os demais dados só podem ser alterados por um administrador.</div>` : '';
    body.innerHTML = headerHTML + avisoRestricaoHTML + sectionsHTML;

    if(!podeEditarFoto){
      const camposLiberados = ['atividades','noPrazo','carga'];
      layoutEquipe.flat().forEach(k=>{
        if(camposLiberados.includes(k)) return;
...
function openReceberModal(id){
  ...
  document.getElementById('modal-title').textContent = 'Registrar recebimento';
  const modalEl = document.querySelector('.modal');
  ...
  const calcTotal = () => {
    const multa = parseFloat(document.getElementById('rec_multa').value) || 0;
    const juros = parseFloat(document.getElementById('rec_juros').value) || 0;
    const desconto = parseFloat(document.getElementById('rec_desconto').value) || 0;
    return valorOriginal + multa + juros - desconto;
  };

  body.innerHTML = `
    <div class="receipt-info-card">
      <div class="receipt-info-item"><span>Cliente</span><strong>${item.clienteNome || clienteNome(item.clienteId)}</strong></div>
      <div class="receipt-info-item"><span>Serviço</span><strong>${item.descricao || '—'}</strong></div>
      <div class="receipt-info-item"><span>Valor original</span><strong>${fmtBRL(valorOriginal)}</strong></div>
    ...
    <div class="form-section" style="border-top:none;padding-top:2px;margin-top:2px">
      <div class="form-section-title">${ICONS.dollar || ''}<span>Composição do valor</span></div>
      <div class="row-3">
        <div class="field"><label>Multa</label><input id="rec_multa" type="number" step="0.01" value="0"></div>
        <div class="field"><label>Juros</label><input id="rec_juros" type="number" step="0.01" value="0"></div>
        <div class="field"><label>Desconto</label><input id="rec_desconto" type="number" step="0.01" value="0"></div>
      </div>
      <div class="field"><label>Valor a receber</label><div class="view-value receipt-total" id="rec_total">${fmtBRL(valorOriginal)}</div></div>
    </div>

    <div class="form-section">
      <div class="form-section-title">${ICONS.dollar || ''}<span>Dados do recebimento</span></div>
      <div class="row-2">
        <div class="field"><label>Forma de recebimento</label>
          <select id="rec_forma">${FORMA_RECEBIMENTO_OPTIONS.map(([v,l])=>`<option value="${v}">${l}</option>`).join('')}</select>
        </div>
        <div class="field"><label>Conta financeira</label>
          <select id="rec_conta">${CONTA_FINANCEIRA_OPTIONS.map(([v,l])=>`<option value="${v}">${l}</option>`).join('')}</select>
        </div>
      </div>
      <div class="row-2">
        <div class="field"><label>Data do recebimento</label><input id="rec_data" type="date" value="${todayISO()}"></div>
        <div class="field"><label>Referência</label><div class="view-value">${finReferencia(item)}</div></div>
      </div>
      <div class="field"><label>Valor recebido</label><input id="rec_valor_recebido" type="number" step="0.01" value="${valorOriginal.toFixed(2)}"></div>
    </div>
  `;

  // Enquanto o usuário não mexer manualmente no "valor recebido", ele acompanha
  // o total calculado a partir de multa/juros/desconto.
  let valorRecebidoManual = false;
  const totalEl = document.getElementById('rec_total');
  const valorRecebidoEl = document.getElementById('rec_valor_recebido');
  const atualizarTotal = () => {
    const total = calcTotal();
    totalEl.textContent = fmtBRL(total);
    if(!valorRecebidoManual) valorRecebidoEl.value = total.toFixed(2);
  };
  ['rec_multa','rec_juros','rec_desconto'].forEach(k=>{
    document.getElementById(k).addEventListener('input', atualizarTotal);
  });
  valorRecebidoEl.addEventListener('input', () => { valorRecebidoManual = true; });

  document.getElementById('modal-foot').innerHTML = `
    <div class="modal-foot-split">
      <button type="button" class="link-btn" id="rec-remover-mj">Remover multa e juros</button>
      <div style="display:flex;gap:10px">
        <button class="btn ghost" id="modal-cancel" type="button">Cancelar</button>
        <button class="btn" id="rec-confirmar" type="button">Registrar recebimento</button>
      </div>
    </div>`;
  document.getElementById('modal-cancel').addEventListener('click', closeModal);
  document.getElementById('rec-remover-mj').addEventListener('click', () => {
    document.getElementById('rec_multa').value = 0;
    document.getElementById('rec_juros').value = 0;
    valorRecebidoManual = false;
    atualizarTotal();
  });
  document.getElementById('rec-confirmar').addEventListener('click', async () => {
    item.multa = parseFloat(document.getElementById('rec_multa').value) || 0;
    item.juros = parseFloat(document.getElementById('rec_juros').value) || 0;
    item.desconto = parseFloat(document.getElementById('rec_desconto').value) || 0;
    item.valorRecebido = parseFloat(valorRecebidoEl.value) || calcTotal();
    item.formaRecebimento = document.getElementById('rec_forma').value;
    item.contaFinanceira = document.getElementById('rec_conta').value;
    item.dataPagamento = document.getElementById('rec_data').value || todayISO();
    item.status = 'pago';
    const ok = await persist('financeiro');
    if(ok){ toast('Recebimento registrado com sucesso.'); closeModal(); renderContent(); }
  });
...
async function saveForm(mod, editId){
  ...
  if(mod==='servicos'){
    ...
    if(valorNum > 0){
      ...
      finObj.clienteNome = clienteNome(obj.clienteId);
